...
当前范围为本机上下文，允许资源、产物与入口位于不同本地磁盘。
远程机器或服务的信息可以保存在人或 AI 维护的资源描述中，但不把远程地址
作为需要拉取、遍历或合并的索引；不做跨机器路径映射或索引同步。

## Organization and Local Setup

2026-09-09 确认的交互与组织方向，具体交付状态见
[005 Spec](../specs/005-context-cli/spec.md)，不代表下列能力已经全部实现。

- 显式初始化不是使用前提。首次成功的添加资源、创建组或配置操作可以在选定位置
  建立上下文；默认放在当前目录的 `.devmeld/`。已有上下文可从当前目录向上查找最近
  的一套，显式位置优先。不扫描资源、不合并上下文、不默认修改项目指引文件。
- 资源的内部身份、逻辑组织路径与源文件位置是不同事实。用户通过组织路径管理
  资源；移动分类不移动源文件、不改变资源身份和已有关系。
- 组支持嵌套，可有独立说明。tag 用于分类，多维度描述用于表达具名信息，二者分开；
  团队可扩展字段，不要求每增加一个字段就修改核心源码。它们不自动授予执行或访问权限。
- 组是否向下传递与子项是否接收父组信息是两个独立选择，双方允许才继承。
  两个方向都有可修改的配置默认值，建议初始值分别为允许传递、不接收继承。
  创建时显式选项优先，否则采用当时的默认值并保存；修改默认值不追溯修改已有项。
- tag 继承合并去重，多维度描述同名字段以子项明确值优先；继承来源须可检查。
  组的整体说明、身份和源位置不自动继承。继承是派生关系，下次同步重新计算，
  不是把父组字段复制成子项自行维护的声明。
- 注册及配置保存、生成内容同步、项目入口接入、Agent 实际读取是不同阶段，
  命令反馈不得把其中一个阶段的成功当成其余阶段已经完成。
